// Um Posto de combustíveis deseja determinar qual de seus produtos tem a preferência de seus clientes. Escreva
//um algoritmo para ler o tipo de combustível abastecido (codificado da seguinte forma: 1.Álcool 2.Gasolina 3.Diesel
//4.Fim). Caso o usuário informe um código inválido (fora da faixa de 1 a 4) deve ser solicitado um novo código (até
//que seja válido). O programa será encerrado quando o código informado for o número 4. Deve ser escrito a
//mensagem: "MUITO OBRIGADO" e a quantidade de clientes que abasteceram cada tipo de combustível.

import java.util.Locale;
import java.util.Scanner;

public class Main {

	public static void main(String[] args) {
		Locale.setDefault(Locale.US);
		Scanner sc = new Scanner (System.in);
		int codProduto , Alcool = 0, Gasolina = 0 , Diesel=0;
		
		System.out.println("Digite o Cod do produto desejado, sendo as opções:");
		System.out.println("1 : Álcool");
		System.out.println("2 : Gasolina");
		System.out.println("3 : Diesel");
		codProduto = sc.nextInt();
		
		while ( codProduto != 4) {

			if (codProduto==1) {
				Alcool = Alcool + 1;
			}
			if (codProduto==1) {
				Gasolina = Gasolina + 1;
			}
			if (codProduto==1) {
				Diesel = Diesel + 1;
			}
			System.out.println("Teste Digite o Cod do produto desejado, sendo as opções:");
			System.out.println("1 : Álcool");
			System.out.println("2 : Gasolina");
			System.out.println("3 : Diesel");
			codProduto = sc.nextInt();
			
		}

		System.out.println("Muito Obrigado ! Alcool: " + Alcool + " Gasolina: " + Gasolina + " Diesel: " + Diesel);
		sc.close();	
}
}
